...
- **Acceptance criteria exist**: Every requirement has at least one EARS-format acceptance criterion. For English specs, scan for "When/If/While/Where/The system shall" statements. For Japanese specs, scan for localized EARS fixed phrases such as "が起きたとき", "の場合", "の間", "を含む場合", or "は常に", and verify the criterion also uses mandatory wording such as "しなければならない" or state-continuation wording such as "し続けなければならない".
