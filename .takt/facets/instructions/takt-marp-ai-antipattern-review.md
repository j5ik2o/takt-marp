...
3. command に応じて reviewed scope を特定してください。`plan` と `compose` は `command-work-report` と source artifacts、`polish` は `render-evidence`、`deliver` は `delivery-artifacts` を主範囲にしてください。
