...
3. 以下の追加観点で確認してください。
   - (a) **style 定義・文書化の存在照合**: plan の各 Layout(custom 句含む)と `SLIDES.md` の各 `_class:` に対応する style 定義(front matter CSS)と deck-local `design-system.md` の文書化(`takt-marp-design-system` の文書化規約: class 名・用途・構造・使用スライド番号)が存在するか確認してください。未定義は **blocker** です。
