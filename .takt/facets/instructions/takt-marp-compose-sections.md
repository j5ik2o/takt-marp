...
4. 各section fileはfront matterなしのMarp本文断片にしてください。各slideは `---` delimiterで始め、`<!-- slide_id: SNNN -->` を入れてください。
...
6. `Visual: html: ...` のスライドは、section file内にHTML/CSS構造を直接書いてください。placeholderにしないでください。
7. `Visual: svg: ...` / `inline-svg: ...` / `existing: ...` のスライドは、section file内に `<!-- Visual: ... -->` placeholder を置いてください。SVG markup と既存画像参照は generate_visuals が所有します。
8. `Visual: none` のスライドにはvisual placeholderを置かないでください。
9. render output の生成や品質判定は行わず、source artifact の作成に集中してください。
