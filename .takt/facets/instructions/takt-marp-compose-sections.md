...
6. `brief.normalized.md` の `Event Context` の `Name` が「未指定」以外の場合は、title slide相当のsection本文に反映してください。`Speaker Profile` が「未指定」以外の場合は、自己紹介相当slideまたはtitle slideのspeaker contextに反映してください。値が「未指定」の項目・節は反映せず、捏造・補完してはなりません。
