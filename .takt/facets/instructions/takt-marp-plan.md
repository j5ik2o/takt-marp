...
18. marker の `design_contract` を読み、marker の `design_contract.path` が指す Resolved Design Contract JSON を必ず開いてください。Resolved Design Contract の source path、namespace、fingerprint、token summary、adherence availability、token constraints、`guidance`、`source_catalog` を `plan.md` と `slide-blueprint.md` に記録してください。`guidance.documents` の `SKILL.md` / `readme.md`、`guidance.component_prompts`、`source_catalog` の components / cards / templates / sample_slides / guidelines / assets は、その deck の Design System 固有の設計入力です。特定ドメインや特定 component 名を固定せず、brief と一致する reusable element / sample / template がある場合だけ選定し、選定理由と使わない理由を残してください。`design_contract` がない、path が読めない、または brief の要求と token constraints / guidance が両立しない場合は、推測で補完せず finding または `needs_input` にしてください。
