...
public `research` command の full research workflow と、private reuse workflow を同じ template source から選ぶための契約。full research は bundled/ejected のどちらでも実行時 staging で callable な built-in deep research wrapper を使い、reuse は同じ source の sibling workflow を使う。
...
Claude Design Source を取り込み、source metadata、fingerprint、token、Design System Guidance、Source Catalog を付与した workflow 実行時の Design Contract。`contract_sha256` は source path や raw zip hash を除いた正規化済み契約内容の fingerprint とし、source path / `source_sha256` は provenance として別に保持する。`plan` と `compose` が同じ design input を参照したことを検証するための snapshot である。
