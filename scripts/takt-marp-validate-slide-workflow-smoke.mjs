...
async function main() {
  ...
  if (targetInfo) {
    ...
    const summaryChecks = [
      ...
      pass("summary:workflow-smoke-boundary", "Smoke summary marks workflow smoke as wiring/state validation, not DDD lecture content acceptance."),
...
async function writeSummary(summaryPath, data) {
  ...
  const content = [
    ...
    "acceptance_scope: workflow-wiring-only",
    "content_quality_evidence: false",
    `slides_source: ${data.target}/SLIDES.md`,
    `pdf_output: dist/${data.target.replace(/^slides\//, "")}/SLIDES.pdf`,
    ...
    "## Scope Boundary",
    "",
    "- This is workflow smoke validation evidence for routing, state gates, template selection, Design Contract wiring, render evidence, delivery artifacts, and rerun/force behavior.",
    "- This is not DDD lecture content acceptance and must not be used as human-facing lecture quality evidence.",
    `- Input SLIDES.md origin when delivery runs: ${data.target}/SLIDES.md generated by the smoke workflow sequence.`,
    `- PDF origin when delivery runs: dist/${data.target.replace(/^slides\//, "")}/SLIDES.pdf generated from that smoke SLIDES.md.`,
    "- Use `npm run slide:content-acceptance` for the deterministic DDD lecture slice quality check.",
    "",
    "## Provider Evidence",
    "",
