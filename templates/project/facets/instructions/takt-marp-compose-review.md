...
3. 以下の追加観点で確認してください。
   - (a) **Design Contract 照合**: marker、`plan.md`、`slide-blueprint.md` の contract fingerprint が一致することを確認してください。`SLIDES.md` の各 `_class:` と front matter CSS が Resolved Design Contract の token constraints に対応しているか確認してください。fingerprint 不一致、style 定義欠落、または token constraints と無関係な class 追加は **blocker** です。
