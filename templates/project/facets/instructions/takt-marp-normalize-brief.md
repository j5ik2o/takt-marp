...
4. **発表コンテキストの確認(必須):** `brief.md` の `## Event` 見出しからイベント名と発表時間(Duration)を確認してください。`Duration` は本編の発表時間を**分単位の数値**(例: `30`)へ正規化して記載し、時間帯表記(例: 14:00〜14:30)や質疑込み表記は数値へ換算したうえで元の記述を Non-blocking notes に残してください。数値へ換算できない場合は「未指定」としてください。イベント名・発表時間が欠落している場合は、正規化結果の `Event Context` 節の該当項目に「未指定」と明記し、Non-blocking notes にも残してください。
