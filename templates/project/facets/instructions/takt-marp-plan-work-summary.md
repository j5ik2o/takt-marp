...
5. `plan.md` と `slide-blueprint.md` に Design Contract section があり、`contract_sha256`、Design Brief status/fingerprint、token summary、brand fonts、adherence availability、`guidance`、`source_catalog` が記録されていることを確認してください。marker の `design_contract.fingerprint.contract_sha256` と不一致、available な Design Brief fingerprint の不一致、Design Brief missing 時の drift protection unavailable note 欠落、または必須 metadata 欠落があれば work issue として記録してください。
