...
スライド一般の品質基準は `takt-marp-general-slide-quality`、visual構成の品質基準は `takt-marp-visual-composition`、worker の禁止事項は `takt-marp-worker-boundary` に委譲する。
...
- `brief.md`、`brief.normalized.md`、`reference-analysis.md`、`plan.md`、`slide-blueprint.md`、Resolved Design Contract、`sections/*`、`SLIDES.md`、HTML visual、`images/*` の境界

**このポリシーが扱わないこと:**
- 1スライド1メッセージ、情報密度、finding severity などの一般品質
- HTML/CSS visual とSVGの使い分け、viewBox、図形内テキスト、表示サイズ、visual fidelity
...
- `sections/*.md` はfront matterを持たない本文断片とし、`SLIDES.md` のassembly元として扱う。
...
- 図が小さい問題は、画像サイズやSVGサイズだけで解決せず、HTML/CSS構造、1列/2列の切り替え、列比率の見直しを先に検討する。
...
- `sections/` と `SLIDES.md` のslide ID、順序、visual参照を同期する。
